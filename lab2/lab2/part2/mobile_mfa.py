...
class BioConnect:
	...
	def sendStepup(self,
		...
		if result == False:
			...
			print(json.dumps(data))
			print(result.content)
			sys.exit("Error: unable to push an authentication request to the mobile app")

		try:
			# Parse the JSON reply
			reply = json.loads(result.content.decode('utf-8'))

			# Extract the verificationId
			user_verification = reply.get("user_verification", {})
			self.verificationId = user_verification.get("uuid","")
	...
	def getStepupStatus(self):
		...
		if result == False:
			...
			print(headers)
			print(result.content)
			sys.exit("Error: unable to get StepupStatus")
		...
		try:
			...
			print(headers)
			print(result.content)
			sys.exit("Error: unexpected reply for getStepupStatus")
